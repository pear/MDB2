...
function odbc_fetch_row_wrapper($result) {
...
// {{{ Class MDB2_Driver_odbc
/**
 * MDB2 odbc Server driver
 *
 * @package MDB2
 * @category Database
 * @author  Frank M. Kromann <frank@kromann.info> rewritten for ODBC by Philipp Schellhaas (mail@pschellhaas.de)
 */
class MDB2_Driver_odbc extends MDB2_Driver_Common
{
    ...
    function errorInfo($error = null, $connection = null)
    {
        ...
        $native_code = null;
        if ($connection) {
            $result = @odbc_query('select @@ERROR as ErrorCode', $connection);
            if ($result) {
                $native_code = @odbc_result($result, 0, 0);
	...
	$params = array($this->database_name,$this->dsn[username],$this->dsn[password]);
        ...
        //odbc_query('SET ANSI_NULL_DFLT_ON ON', $connection);
       ...
       if ((bool)ini_get('odbc.datetimeconvert')) {
           @ini_set('odbc.datetimeconvert', '0');
       }

	   /*
       if (empty($this->dsn['disable_iso_date'])) {
           @odbc_query('SET DATEFORMAT ymd', $connection);
       }
       */
    ...
    function connect()
    {
        ...
        if ($this->database_name) {
            if ($this->database_name != $this->connected_database_name) {
            	/*
                if (!odbc_select_db($this->database_name, $connection)) {
   ...
   /**
    ...
    function _affectedRows($connection, $result = null)
    {
        ...
        return odbc_rows_affected($connection);
    }
    ...
    function _checkSequence($seq_name)
    {
        ...
        return odbc_result($tableExists, 0, 0);
    }
    ...
    function _skipLimitOffset()
    {
        ...
        if ($this->offset) {
            while ($this->offset_count < $this->offset) {
                ++$this->offset_count;
                if (!is_array(odbc_fetch_row_wrapper($this->result))) {
...
// }}}
// {{{ class MDB2_BufferedResult_odbc
